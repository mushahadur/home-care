...
<main class="bg-gray-50 min-h-screen pb-12">
  ...
  <div class="bg-gradient-to-r from-[#115c7e] to-[#1a7a9e] py-8 md:py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <!-- Stats Cards -->
      <div class="justify-content items-center grid grid-cols-2 md:grid-cols-3 gap-4 max-w-4xl mx-auto">
        <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center">
          <p class="text-2xl font-bold text-white">{{$orders->count()}}</p>
          <p class="text-xs text-blue-100">Total Orders</p>
        </div>
        <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center">
          <p class="text-2xl font-bold text-white">{{$orders->where('status', 'completed')->count()}}</p>
          <p class="text-xs text-blue-100">Completed</p>
        </div>
        <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center">
          <p class="text-2xl font-bold text-white">{{$orders->where('status', 'cancelled')->count()}}</p>
          ...
          <div class="flex-1 p-5 sm:p-6 flex flex-col">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm mb-5">
              ...
              <div>
                ...
                <p class="font-bold text-[var(--accent)]">৳ {{ number_format($order->total_price, 2) }}</p>
              </div>
            </div>
            ...
            <!-- Actions -->
              ...
              <button type="submit"
                class="inline-flex items-center px-5 py-2.5 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition text-sm font-medium border border-red-200">
